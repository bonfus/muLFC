...
class TestLFCExtension(unittest.TestCase):
    ...
    def test_rotation_of_cart_coord(self):
        ...
        # Too many transpositions here...
        np.testing.assert_array_almost_equal(c,np.dot(mrmat,cr.T).T)
        np.testing.assert_array_almost_equal(d,np.dot(mrmat,dr.T).T)
        np.testing.assert_array_almost_equal(l,np.dot(mrmat,lr.T).T)
    ...
    def test_multiple_muons(self):
        ...
        ####  versions
        mc,md,ml = lfclib.Simple(p,fc,k,phi,mu,scell,latpar,r,nnn,rc)
        i=0
        for m in mu:
          sc,sd,sl = lfclib.Fields('s', p,fc,k,phi,m,scell,latpar,r,nnn,rc)
          np.testing.assert_array_almost_equal(mc[i], sc[0])
          np.testing.assert_array_almost_equal(md[i], sd[0])
          np.testing.assert_array_almost_equal(ml[i], sl[0])
          i+=1

        mc,md,ml = lfclib.Fields('r', p,fc,k,phi,mu,scell,latpar,r,nnn,rc,12,ax)
        mc = mc.reshape([30,12,3])
        md = md.reshape([30,12,3])
        ml = ml.reshape([30,12,3])
        i=0
        for m in mu:
        ...
        mc = mc.reshape([30,8,3])
        md = md.reshape([30,8,3])
        ml = ml.reshape([30,8,3])
        i=0
        for m in mu:
